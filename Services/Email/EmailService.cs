...
namespace HandsForPeaceMakingAPI.Services.Email
{
    public class EmailService
    {
        ...
        public async Task EnviarCorreo(string token, string correo, string Nombres, string user)
        {
            ...
            email.From.Add(new MailboxAddress("HANDS FOR PEACE MAKING", _smptConfig.Email));
        ...
        public async Task ConfirmarCambioContraseña(string user, string correo, string Nombres)
        {
            var email = new MimeMessage();
            email.From.Add(new MailboxAddress("HANDS FOR PEACE MAKING", _smptConfig.Email));
            email.To.Add(new MailboxAddress(Nombres, correo));
            email.Subject = "Contraseña restablecida HFPMApp";

            var htmlBody = $@"
                            <html>
                            <body>
                                <h1>HOLA {Nombres} </h1>
                                <p>Hemos restablecido tu contraseña</p>
                                <p>El usuario {user} fue el que solicito el restablecimiento </p>
                                <ul>
                                    <li>Nombres: {Nombres}</li>
                                    <li>Usuario: {user}</li>
                                </ul>
                                <p>Ahora puedes usar tu nueva contraseña para iniciar sesion en la aplicacion HFPMApp</p>
                            </body>
                            </html>";
